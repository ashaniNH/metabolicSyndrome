...
library(caret)         # Confusion matrix, performance metrics
library(pROC)          # ROC and AUC
library(ggplot2)       # Plots
library(tidyr)         # Data reshaping for plotting
...
rf_model <- randomForest(MetabolicSyndrome ~ .,
                         ...
                         ntree = 500,
                         mtry = floor(sqrt(ncol(trainingSet_clean) - 1)),
                         importance = TRUE,
                         na.action = na.omit)
...
train_pred_class <- predict(rf_model, trainingSet_clean, type = "response")

# Test predictions
test_pred_prob <- predict(rf_model, testingSet_clean, type = "prob")[, 2]
test_pred_class <- predict(rf_model, testingSet_clean, type = "response")
...
# AUC calculations
train_auc <- auc(roc(trainingSet_clean$MetabolicSyndrome, train_pred_prob))
test_auc <- auc(roc(testingSet_clean$MetabolicSyndrome, test_pred_prob))
...
knitr::kable(comparison, digits = 3, caption = "Random Forest Performance Comparison")
